...
@SpringBootTest
public class RegistrationControllerTest10 {
    @Autowired
    ...
    private void setUp() throws OverLimitGroupException, AssignmentException,
        ...
        assignedSubject.setNumberOfGroup(1);
        Mockito.doThrow(new OverLimitRegistrationException(assignedSubject))
    ...
    public void testFindAllEnableTermSubject(){
        ...
        try {
            mockMvc.perform(requestBuilder)
                    ...
                    .andExpect(jsonPath("$"
                            ,containsString("Bạn chỉ được phép đăng kí tối đa 1 nhóm cho môn học này!")));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
